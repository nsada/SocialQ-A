	...
	<br>
 	排名可见性：
		...
		<s:iterator value="selections">   
			...
			<%i++; %>
			<s:if test = '#A!=""'><p>A:<s:property value = "A"/></p></s:if>
			<s:if test = '#B!=""'><p>B:<s:property value = "B"/></p></s:if>
			<s:if test = '#C!=""'><p>C:<s:property value = "C"/></p></s:if>
			<s:if test = '#D!=""'><p>D:<s:property value = "D"/></p></s:if>
			<s:if test = '#E!=""'><p>E:<s:property value = "E"/></p></s:if>
			<s:if test = '#F!=""'><p>F:<s:property value = "F"/></p></s:if>
        ...
        <div>
      		 <a style="float:right" href=" /SocialQ-A/deleteexam.action?ExamID=6"> <button type="button">取消编辑试卷</button>  </a>
     		 <input style="float:right" type="button" onclick="updateexam()" value="提交生成我的试卷">
      		 <a style="float:right" href=" /SocialQ-A/Find.action?ExamID=6">  <button type="button">插入题目</button> </a>
    	 </div>
    </form>
	</div>
